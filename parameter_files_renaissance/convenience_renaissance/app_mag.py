...
num_filters = len(os.listdir(filter_directory))
ab_mag_UV = np.empty(flux.shape[0], num_filters)
...
for i in range(flux.shape[0]):
    for filters in os.listdir(filter_directory):
        ...
        micron = np.asarray(micron) * (1.+z)
        throughput = np.asarray(throughput) * (1.+z)
        ...
        ab_mag_UV[i][x] = apparent_magnitude
        del micron
        del throughput
        micron = []
        throughput = []
